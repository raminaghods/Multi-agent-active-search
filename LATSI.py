...
code for the submitted work: "Asynchronous Multi Agent Active Search for Sparse Signals
with Region Sensing"
...
#        beta_tilde = np.maximum(np.reshape(self.rng.multivariate_normal(np.squeeze(beta_hat),Sig_beta),(self.n,1)),np.zeros((self.n,1)))
        ...
        beta_tilde = np.maximum(self.gibbs_invgauss(1000,XT_X,XT_Y),np.zeros((self.n,1)))
...
#                print("so far detected: ",detected," left:",np.count_nonzero(beta_rsi))
    ...
    def gibbs_invgauss(self,itr,XT_X,XT_Y):
        ...
        for i in range(itr):
            ...
            beta = np.random.multivariate_normal(np.squeeze(np.matmul(Sig,XT_Y)),self.sigma2*Sig)
            for j in range(self.n):
...
#                    bestinv = b
        ...
        if(avg_IG_RSI==0):
            ...
            reward = IG_RSI/avg_IG_RSI - (self.alpha * (loss/avg_loss))
    ...
    #        print('sensing matrix:',np.transpose(bestx))
        ...
        if not qinfo.compute_posterior:
            ...
            result = {'x':X, 'y':Y, 'par':L, 'pre-eval':True}
        else:
